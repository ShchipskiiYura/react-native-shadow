...
export default class BoxShadow extends Component {
	render = () => {
		...
		const {fromTransaction} = this.props
		...
		// fromTrasactionYura = rectHeight < 0 || rectWidth < 0
		//update 2017-03-04
		//format the color
		...
		return (
			<View style={[{position:"relative",width:width,height:height},style]}>
				<Svg height={height+lineWidth*2+radius*2} width={width+lineWidth*2+radius*2} style={{position:"absolute",top:y-lineWidth,left:x-lineWidth}}>
					...
					{fromTransaction ? null : <Path d={`M ${outerWidth} ${lineWidth},h ${rectWidth},q ${radius} 0 ${radius} ${radius},v ${rectHeight},q 0 ${radius} -${radius} ${radius},h -${rectWidth},q -${radius} 0 -${radius} -${radius},v -${rectHeight},q 0 -${radius} ${radius} -${radius}`} fill={`rgba(${rgb[0]},${rgb[1]},${rgb[2]},${opacity || 1})`}/>}
				</Svg>
				{children}
			</View>
		)
